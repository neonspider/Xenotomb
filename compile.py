...
parser.add_argument("-d", "--debug", help="compiles scripts in debug mode",
                    action="store_true")
args = parser.parse_args()
if args.debug:
	xenotomb_debug = '--define XENO_DEBUG'
	else:
	xenotomb_debug = '--define NDEBUG'

if debug_mode == '--debug' or debug_mode == '-d':
	xenotomb_debug = '--define XENO_DEBUG'
	else:
	xenotomb_debug = '--define NDEBUG'
...
if not gdcc_dir.is_dir():
	print('GDCC must be in a directory adjacent to this script for compilation to work!')
	sys.exit()

# delete all binaries and maps
print('\nClearing intermediary compilation files...')
