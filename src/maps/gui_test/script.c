...
void main(void)
{
	...
	for(;;)
	{
		...
		ACS_SpawnForced(s"GuiShooter", ACS_GetActorX(player_tid), ACS_GetActorY(player_tid),
			ACS_GetActorZ(player_tid) + ACS_GetActorViewHeight(player_tid) + zoffset_hack, shooter_tid); // spawn hitscan shooter at player position
		
		ACS_LineAttack(shooter_tid, ACS_GetActorAngle(player_tid), ACS_GetActorPitch(player_tid), 0, s"GuiCursor", s"None", range, FHF_NORANDOMPUFFZ | FHF_NOIMPACTDECAL, cursor_tid);
